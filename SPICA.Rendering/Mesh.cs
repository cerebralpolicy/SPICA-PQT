...
namespace SPICA.Rendering
{
    public class Mesh : IDisposable
    {
        ...
        internal H3DMaterial Material => Parent.BaseModel.Materials[BaseMesh.MaterialIndex];
        ...
        public void Render()
        {
            ...
            if (Texture0Name != null)
            {
                ...
                {
                    if (Parent.Renderer.TryBindTexture(3, Texture0Name))
                        SetWrapAndFilter(TextureTarget.TextureCubeMap, 0);
                }
                else
                {
                    if (Parent.Renderer.TryBindTexture(0, Texture0Name))
            ...
            {
                if (Parent.Renderer.TryBindTexture(1, Texture1Name))
                SetWrapAndFilter(TextureTarget.Texture2D, 1);
            }

            if (Texture2Name != null)
            {
                if (Parent.Renderer.TryBindTexture(2, Texture2Name))
            ...
            //Sub mesh culling
            if (BaseMesh.SubMeshes.Count == 0)
            {
                var index = Parent.BaseModel.Meshes.IndexOf(BaseMesh);
                var mesh = Parent.BaseModel.SubMeshCullings[index];
        ...
        private static All GetMinFilter(H3DTextureMinFilter Filter)
        {
            switch (Filter)
            {
                ...
                case H3DTextureMinFilter.NearestMipmapNearest: return All.NearestMipmapNearest;
                case H3DTextureMinFilter.NearestMipmapLinear:  return All.NearestMipmapLinear;
                case H3DTextureMinFilter.Linear:               return All.Linear;
                case H3DTextureMinFilter.LinearMipmapNearest:  return All.LinearMipmapNearest;
                case H3DTextureMinFilter.LinearMipmapLinear:   return All.LinearMipmapLinear;
